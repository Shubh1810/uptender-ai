...
export default function OnboardingPage() {
  ...
  React.useEffect(() => {
    // Initialize from URL param if provided (e.g., /onboarding?step=2)
    const stepParam = searchParams?.get('step');
    if (stepParam === '1' || stepParam === '2' || stepParam === '3') {
      setStep(parseInt(stepParam, 10) as Step);
    }

    // If user is already authenticated, move to step 2
    supabase.auth.getUser().then(({ data }) => {
      // Only auto-move to step 2 if no explicit step param is set
