...
export default function SearchPage() {
  ...
  return (
    <div className="flex-1 p-4 pt-2">
        ...
        <form onSubmit={handleSearch} className="flex gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-gray-300 dark:border-white/[0.12] rounded-lg bg-white dark:bg-black/20 text-gray-900 dark:text-white/90 placeholder:text-gray-400 dark:placeholder:text-white/40 focus:outline-none focus:ring-2 focus:ring-gray-300 dark:focus:ring-white/[0.16] focus:border-transparent transition-all"
            />
          </div>
          <Button
            type="submit"
            disabled={loading}
            className="px-8 py-2.5 bg-gray-900 hover:bg-gray-800 dark:bg-white dark:hover:bg-white/90 text-white dark:text-black font-medium"
          >
            {loading ? (
              <>
      ...
      {/* Error State */}
      {error && (
        <div className="bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded-lg p-4 mb-6 flex items-start gap-3">
          ...
          <div>
            ...
            <Button
              ...
              className="mt-3 text-xs px-4 py-2 h-8 bg-red-600 hover:bg-red-700 dark:bg-red-500 dark:hover:bg-red-600 text-white"
            >
              Try Again
            </Button>
          </div>
        ...
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <div className="relative w-32 h-32 mb-6">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-2xl font-bold text-gray-900 dark:text-white">{Math.round(loadingProgress)}%</span>
            </div>
          </div>
          
          <p className="text-gray-600 dark:text-white/70 font-medium mb-2">Loading tenders...</p>
          <p className="text-sm text-gray-500 dark:text-white/50">Fetching from database snapshot...</p>
        </div>
      )}
      ...
      {!loading && !error && tenders.length === 0 && (
        <div className="text-center py-20">
          <FileText className="h-16 w-16 text-gray-300 dark:text-white/[0.15] mx-auto mb-4" />
          <p className="text-gray-500 dark:text-white/60 mb-2 font-medium">No tenders found</p>
          <p className="text-sm text-gray-400 dark:text-white/40">Try adjusting your search terms</p>
        </div>
      )}
          ...
          {tenders.map((tender, index) => (
            ...
            >
              ...
              <div className="flex items-start justify-between mb-4">
                ...
                >
                  <Button className="bg-gray-900 hover:bg-gray-800 dark:bg-white dark:hover:bg-white/90 text-white dark:text-black text-sm px-4 py-2 h-9">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
                <div className="flex items-start gap-2">
                  <FileText className="h-3.5 w-3.5 text-gray-400 dark:text-white/30 mt-0.5" />
                  <div>
                    <p className="text-xs text-gray-500 dark:text-white/40 mb-0.5">Reference No.</p>
                    <p className="text-sm font-medium text-gray-900 dark:text-white/80">{tender.ref_no}</p>
                  </div>
                </div>

                <div className="flex items-start gap-2">
                  <Calendar className="h-3.5 w-3.5 text-gray-400 dark:text-white/30 mt-0.5" />
                  <div>
                    <p className="text-xs text-gray-500 dark:text-white/40 mb-0.5">Published</p>
                ...
                <div className="flex items-start gap-2">
                  <Calendar className="h-3.5 w-3.5 text-gray-400 dark:text-white/30 mt-0.5" />
                  <div>
                    <p className="text-xs text-gray-500 dark:text-white/40 mb-0.5">Opening Date</p>
                    <p className="text-sm font-medium text-gray-900 dark:text-white/80">
                ...
                <div className="flex items-start gap-2">
                  <Calendar className="h-3.5 w-3.5 text-gray-400 dark:text-white/30 mt-0.5" />
                  <div>
                    <p className="text-xs text-gray-500 dark:text-white/40 mb-0.5">Closing Date</p>
                    <p className="text-sm font-medium text-red-600 dark:text-white/70">
              ...
              <div className="flex items-center justify-between pt-3.5 border-t border-gray-100 dark:border-white/[0.06]">
                <div className="flex gap-2">
                  <span className="px-2.5 py-1 bg-gray-100 dark:bg-white/[0.08] text-gray-700 dark:text-white/70 text-xs rounded-md font-medium">
                    Active
                  </span>
                  <span className="px-2.5 py-1 bg-gray-50 dark:bg-white/[0.04] text-gray-600 dark:text-white/60 text-xs rounded-md font-medium">
                    Government
                  </span>
                </div>
                <Button
                  ...
                  className={`text-xs px-3 py-1.5 h-7 transition-all duration-200 ${
      ...
      {!loading && !error && tenders.length > 0 && (
        <div className="mt-8 flex items-center justify-center gap-2">
          <Button
            ...
            variant="outline"
            className="px-4 py-2 h-9 dark:border-white/[0.12] dark:text-white/70 dark:hover:bg-white/[0.06] dark:hover:border-white/[0.16] disabled:opacity-40 dark:disabled:opacity-30"
          >
            Previous
          </Button>
          <span className="px-4 py-2 text-sm text-gray-600 dark:text-white/60 font-medium">
            Page {currentPage}
          </span>
          <Button
            onClick={() => fetchTenders(currentPage + 1, searchQuery)}
